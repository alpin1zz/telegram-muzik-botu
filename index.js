...
async function startMtprotoClient(chatId) {
    if (mtprotoClient && mtprotoClient.connected) {
        return mtprotoClient;
    }

    console.log("MTProto istemcisi başlatılıyor. Bot hesabınızda oturum açmanız gerekebilir.");
    console.log("Eğer StringSession boşsa, ilk çalıştırmada bir doğrulama kodu isteyecektir.");

    mtprotoClient = new TelegramClient(stringSession, API_ID, API_HASH, { connectionRetries: 5 });

    await mtprotoClient.start({
        phoneNumber: async () => await new Promise(resolve => {
            bot.sendMessage(chatId, 'Lütfen botunuzun telefon numarasını girin (ülke koduyla birlikte, örn: +905XXXXXXXXX):');
            bot.once('message', (msg) => {
                resolve(msg.text);
            });
        }),
        password: async () => await new Promise(resolve => {
            bot.sendMessage(chatId, 'Lütfen iki adımlı doğrulama şifrenizi girin (varsa):');
            bot.once('message', (msg) => {
                resolve(msg.text);
            });
        }),
        phoneCode: async () => await new Promise(resolve => {
            bot.sendMessage(chatId, 'Telegram tarafından gönderilen doğrulama kodunu girin:');
            bot.once('message', (msg) => {
                resolve(msg.text);
            });
        }),
        onError: (err) => console.error("MTProto oturum hatası:", err),
    });

    console.log("MTProto istemcisi başarıyla bağlandı. Oturum dizeniz (bunu StringSession'a kaydedin):");
    console.log(mtprotoClient.session.save()); 
...
async function joinVoiceChat(chatId) {
    if (!mtprotoClient || !mtprotoClient.connected) {
        await startMtprotoClient(chatId);
    }

    try {
        const peer = await mtprotoClient.getEntity(chatId); 
        
        voiceChatConnection = {
            chatId: chatId,
            playing: false,
            paused: false,
            setVolume: (vol) => console.log(`[MTProto Simülasyon] Ses seviyesi ${vol} olarak ayarlandı.`),
            play: (stream) => {
                console.log("[MTProto Simülasyon] Ses stream'i başlatıldı. Gerçek Opus paketleri burada gönderilecek.");
                voiceChatConnection.playing = true;
            },
            pause: () => { voiceChatConnection.paused = true; console.log("[MTProto Simülasyon] Duraklatıldı."); },
            resume: () => { voiceChatConnection.paused = false; console.log("[MTProto Simülasyon] Devam ettiriliyor."); },
            leave: async () => { 
                console.log("[MTProto Simülasyon] Sesli sohbetten ayrılıyor.");
                voiceChatConnection = null;
                activeVoiceChats.delete(chatId);
                console.log("[MTProto Simülasyon] Sesli sohbetten ayrıldı.");
            },
        };
        activeVoiceChats.set(chatId, voiceChatConnection);
        return voiceChatConnection;

    } catch (error) {
        console.error(`MTProto sesli sohbete katılma hatası (${chatId}): ${error.message}`);
        throw error;
    }
}

async function leaveVoiceChat(chatId) {
    const conn = activeVoiceChats.get(chatId);
    if (conn) {
        try {
            await conn.leave();
        } catch (e) {
            console.error("MTProto leave error:", e);
        }
    }
    activeVoiceChats.delete(chatId);
}
...
async function playSong(msg, songInfo) {
    ...
    try {
        ...
        const streamUrl = ytdl(songInfo.url, {
            filter: 'audioonly',
        ...
        const ffmpegProcess = ffmpeg(streamUrl)
            ...
            .format('ogg') 
            .on('error', (err) => {
                console.error(`FFmpeg hatası (${chatId}): ${err.message}`);
        ...
        console.log("MTProto'da ses stream'i simüle ediliyor. Gerçek gönderme burada yapılacak.");
        ...
        if (songInfo.source === 'youtube' && songInfo.url) {
            const videoId = ytdl.getURLVideoID(songInfo.url);
            initialMessageText += `<a href="http://img.youtube.com/vi/${videoId}/default.jpg">.</a>`;
        }
        ...
        if (messageIdToUpdate) {
            try {
                ...
                console.warn(`Mevcut mesajı güncellerken hata: ${editError.message}. Yeni mesaj gönderiliyor.`);
    ...
    bot.onText(/\/nowplaying/, async (msg) => {
        ...
        if (messageIdToUpdate) {
            try {
                ...
                console.warn(`Mevcut mesajı güncelleyen ${editError.message}. Yeni mesaj gönderiliyor.`);
